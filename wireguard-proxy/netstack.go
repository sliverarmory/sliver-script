// This userspace TUN is derived from Sliver client/transport/wireguard_netstack.go
// at ca685f5eed64c3327c0e57504928cfd2d2e96bea. Keeping the small transport
// adapter here lets the npm package build its helper without an ambient Sliver
// checkout while dependencies stay locked in go.mod and go.sum.
package main
...
func createTransportNetTUN(localAddresses []netip.Addr, mtu int) (tun.Device, *transportNet, error) {
	...
	if tcpipErr := dev.stack.SetTransportProtocolOption(tcp.ProtocolNumber, &tcpRecoveryOpt); tcpipErr != nil {
		return nil, nil, fmt.Errorf("could not disable TCP RACK: %v", tcpipErr)
	}

	renoOpt := tcpip.CongestionControlOption("reno")
	if tcpipErr := dev.stack.SetTransportProtocolOption(tcp.ProtocolNumber, &renoOpt); tcpipErr != nil {
		return nil, nil, fmt.Errorf("could not set TCP congestion control to reno: %v", tcpipErr)
	}
	...
	if tcpipErr := dev.stack.SetTransportProtocolOption(tcp.ProtocolNumber, &tcpRXBufOpt); tcpipErr != nil {
		return nil, nil, fmt.Errorf("could not set TCP RX buffer size: %v", tcpipErr)
	}
	...
	if tcpipErr := dev.stack.SetTransportProtocolOption(tcp.ProtocolNumber, &tcpTXBufOpt); tcpipErr != nil {
		return nil, nil, fmt.Errorf("could not set TCP TX buffer size: %v", tcpipErr)
	}
